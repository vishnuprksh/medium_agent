...
def cached_fetch(feed_urls: tuple[str, ...], max_items_per_feed: int):
    return fetch_articles(feed_urls, max_items_per_feed=max_items_per_feed)
...
if run:
    ...
    with st.status("Agents are exploring Medium...", expanded=True) as status:
        ...
        articles, errors = cached_fetch(tuple(plan.feed_urls), max_items_per_feed=max_items_per_feed)
        ...
        if not articles:
            st.error("No articles found from the selected feeds. Try broader tags such as ai, programming, startups, data-science, or software-development.")
            st.stop()
